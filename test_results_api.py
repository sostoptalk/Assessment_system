...
def test_results_api():
    ...
    # 3. 测试 /results/by-user 接口
    print(f"\n3. 测试 /results/by-user 接口 (user_id={user_id})...")
    ...
    # 4. 测试错误的参数（带斜杠）
    print(f"\n4. 测试错误参数 (user_id={user_id}/)...")
    params_wrong = {"user_id": f"{user_id}/"}
    
    print(f"请求参数: {params_wrong}")
    
    response_wrong = requests.get(url, params=params_wrong, headers=headers)
    print(f"响应状态码: {response_wrong.status_code}")
    
    if response_wrong.status_code == 422:
        print("✅ 正确返回422错误（参数验证失败）")
        print(f"错误详情: {response_wrong.json()}")
    else:
        print(f"❌ 意外的响应: {response_wrong.status_code}")
        print(f"响应内容: {response_wrong.text}")
