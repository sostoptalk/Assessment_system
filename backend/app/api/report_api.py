...
from app.main import SessionLocal, User, Paper, PaperAssignment, Question, PaperQuestion, Dimension
import sys
...
def batch_generate_reports(req: BatchGenerateRequest):
    ...
    for user_id in req.user_ids:
        ...
        threading.Thread(target=_generate_report_task, args=(task_id, user_id, req.paper_id), daemon=True).start()
        task_ids.append(task_id)
    return {"success": True, "task_ids": task_ids}

def _generate_report_task(task_id, user_id, paper_id):
    db = SessionLocal()
    try:
